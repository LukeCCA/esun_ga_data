...
try:
    print('start process query app_event_android...')

    client = bigquery.Client()
    query_job = client.query("""SELECT max(visitDate) as max_date, min(visitDate) as min_date 
    FROM `neat-motif-123006.ga_etl.app_event_android` """)
    ...
    d_dir = '{}app_event_android_{}.D'.format(output_dir, max_date)
    h_dir = '{}app_event_android_{}.H'.format(output_dir, max_date)
    
    with open(d_dir, 'w') as f, open(h_dir, 'w') as f1:
        print('start query')
        query_job = client.query("""SELECT * FROM `neat-motif-123006.ga_etl.app_event_android`""")
        res = query_job.result()
        ...
        f1.write(min_date+
                 ...
                 'app_event_android_{}.D'.format(max_date))
        print('finish query app_event_android')

    # 寫入檢查表
    table_name = 'app_event_android'
