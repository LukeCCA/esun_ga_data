...
try:
    print('start process query web_campaign_impression...')
    client = bigquery.Client()
    query_job = client.query("""SELECT max(visitDate) as max_date, min(visitDate) as min_date FROM `neat-motif-123006.ga_etl.web_campaign_impression`""")
    ...
    num_of_rows = 0
    num_of_process = 0
    d_dir = '{}web_campaign_impression_{}.D'.format(output_dir, max_date)
    h_dir = '{}web_campaign_impression_{}.H'.format(output_dir, max_date) 
    with open(d_dir, 'w') as f, open(h_dir, 'w') as f1:
        print('start query')
        query_job = client.query("""SELECT * FROM `neat-motif-123006.ga_etl.web_campaign_impression`""")
        ...
        for row in res:
            ...
            columns = ['{}_{}'.format(row['visitDate'],num_of_process),
                       row['etl_dt'],row['eventDateTime'],row['visitDateTime'],
                       row['visitDate'],row['fullVisitorId'],row['deviceCategory'],
                       row['hits_eventInfo_eventLabel'],row['clientId']]
        ...
        f1.write(min_date+
                ...
                'web_campaign_impression_{}.D'.format(max_date))
        print('finish query web_campaign_impression')

    # 寫入檢查表
    table_name = 'web_campaign_impression'
    ...
    print('--------------------------------------')
    print('number of row need to processed: {}'.format(num_of_rows))
    print('number of row actually to processed: {}'.format(num_of_process))
