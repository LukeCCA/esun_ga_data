...
try: 
    print('start process query app_custno_mapping_android...')
    ...
    query_job = client.query("""SELECT max(DATE(TIMESTAMP_SECONDS(visitStartTime), "Asia/Taipei")) as max_date,
                                ...
                                FROM `neat-motif-123006.118454301.ga_sessions_*`,
    ...
    num_of_rows = 0
    num_of_process = 0
    d_dir = '{}app_custno_mapping_android_{}.D'.format(output_dir, max_date)
    h_dir = '{}app_custno_mapping_android_{}.H'.format(output_dir, max_date) 
    with open(d_dir, 'w') as f, open(h_dir, 'w') as f1:
        ...
        query_job = client.query("""
             SELECT
              ...
              FROM
                `neat-motif-123006.118454301.ga_sessions_*`,
                UNNEST(customDimensions) AS customDimensions
              WHERE
        ...
        f1.write(min_date+
                 ...
                 'app_custno_mapping_android_{}.D'.format(max_date))
        print('finish query app_custno_mapping_android')


    # 寫入檢查表
    table_name = 'app_custno_mapping_android'
