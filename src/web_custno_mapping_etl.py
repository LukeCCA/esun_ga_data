...
try:
    print('start process query web_custno_mapping...')

    client = bigquery.Client()
    query_job = client.query("""SELECT max(visitDate) as max_date, min(visitDate) as min_date FROM `neat-motif-123006.ga_etl.web_custno_mapping` """)
    ...
    num_of_rows = 0
    num_of_process = 0
    d_dir = '{}web_custno_mapping_{}.D'.format(output_dir, max_date)
    h_dir = '{}web_custno_mapping_{}.H'.format(output_dir, max_date) 

    with open(d_dir, 'w') as f, open(h_dir, 'w') as f1:
        print('start query')
        query_job = client.query("""SELECT * FROM `neat-motif-123006.ga_etl.web_custno_mapping`""")
        ...
        f1.write(min_date+
                 ...
                 'web_custno_mapping_{}.D'.format(max_date))
        print('finish query web_custno_mapping')

    # 寫入檢查表
    table_name = 'web_custno_mapping'
    ...
    open('{}web_custno_mapping_{}.D'.format(output_dir, max_date), 'w')
    with open('{}web_custno_mapping_{}.H'.format(output_dir, max_date), 'w') as f:  
        f.write(max_date+max_date+datetime.datetime.strftime(datetime.datetime.now(pytz.timezone('Asia/Taipei')),'%Y%m%d%H%M%S')+'{:010d}'.format(0)+'web_custno_mapping_{}.D'.format(max_date))
    f.close()
    # 寫入檢查表
    table_name = 'web_custno_mapping'
    df = pd.read_csv('/mnt/esun-crawler-ga/ga_checklist/ga_checklist_{}_{}.csv'.format(table_name, max_date))
