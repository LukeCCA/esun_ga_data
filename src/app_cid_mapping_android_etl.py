...
try:
    ...
    FROM `neat-motif-123006.ga_etl.app_cid_mapping_android`""")
    ...
    d_dir = '{}app_cid_mapping_android_{}.D'.format(output_dir, max_date)
    h_dir = '{}app_cid_mapping_android_{}.H'.format(output_dir, max_date)    
    with open(d_dir, 'w') as f, open(h_dir,'w') as f1:
        print('start query app_cid_mapping_android')
        query_job = client.query("""SELECT * FROM `neat-motif-123006.ga_etl.app_cid_mapping_android`""")
        res = query_job.result()
        ...
        f1.write(min_date+
                 ...
                 'app_cid_mapping_android_{}.D'.format(max_date))
        print('finish query app_cid_mapping_android')

    # 寫入檢查表
    table_name = 'app_cid_mapping_android'
    ...
    print('-------------------------------------------------------------')
    print('app_cid_mapping_android number of row need to processed: {}'.format(num_of_rows))
    print('app_cid_mapping_android number of row actually to processed: {}'.format(num_of_process))
    print('app_cid_mapping_android process time: {:.2f} mins'.format((time.time()-start_time)/60))
    print('-------------------------------------------------------------')
