...
try:
    print('start process query app_native_android...')

    client = bigquery.Client()

    query_job = client.query("""SELECT max(visitDate) as max_date, min(visitDate) as min_date 
    FROM `neat-motif-123006.ga_etl.app_native_android` """)
    ...
    num_of_rows = 0
    num_of_process = 0
    d_dir = '{}app_native_android_{}.D'.format(output_dir, max_date)
    h_dir = '{}app_native_android_{}.H'.format(output_dir, max_date)

    with open(d_dir, 'w') as f, open(h_dir, 'w') as f1:
        print('start query')
        query_job = client.query("""SELECT * FROM `neat-motif-123006.ga_etl.app_native_android`""")
        ...
        f1.write(min_date+
                 ...
                 'app_native_android_{}.D'.format(max_date))
        print('finish query app_native_android')
        
    # 寫入檢查表
    table_name = 'app_native_android'
