...
try: 
    ...
    query_job = client.query("""SELECT max(visitDate) as max_date, min(visitDate) as min_date FROM `neat-motif-123006.ga_etl.app_event_ios` """)
    ...
    num_of_rows = 0
    num_of_process = 0
    d_dir = '{}app_event_ios_{}.D'.format(output_dir, max_date)
    h_dir = '{}app_event_ios_{}.H'.format(output_dir, max_date)
    
    with open(d_dir, 'w') as f, open(h_dir, 'w') as f1:
        print('start query app_event_ios')
        query_job = client.query("""SELECT * FROM `neat-motif-123006.ga_etl.app_event_ios`""")
        res = query_job.result()
        ...
        for row in res:
            ...
            columns = ['{}_{}'.format(row['visitDate'],num_of_process),
                       row['etl_dt'],row['visitDateTime'],row['eventDateTime'],row['visitDate'],
                       row['fullVisitorId'],row['hits_eventInfo_eventCategory'],
                       row['hits_eventInfo_eventAction'],row['hits_eventInfo_eventLabel'],
        ...
        f1.write(min_date+
                 ...
                 'app_event_ios_{}.D'.format(max_date))
        print('finish query app_event_ios')
    


    # 寫入檢查表
    table_name = 'app_event_ios'
